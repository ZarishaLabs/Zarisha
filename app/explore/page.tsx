  ...
  useEffect(() => {
    async function fetchAiring() {
      try {
        ...
        setAnimeList(uniqueAnime.slice(0, 20));
      } catch (err) {
    ...
    <div className="space-y-8">
      ...
      <section className="card p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-[3px]">Explore</h1>
            <p className="text-text-secondary mt-2">Discover popular and trending anime</p>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-section-header uppercase tracking-[2px]">Top 20 Anime</h2>
        </div>
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background via-background/90 to-transparent p-3 pt-8">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 text-xs text-text-secondary">
                    <span className="text-status-label uppercase">{anime.type}</span>
        ...
        {!loading && !error && animeList.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <p className="text-text-secondary">Check back later for new releases</p>
          </div>
        )}
      </section>
    </div>
